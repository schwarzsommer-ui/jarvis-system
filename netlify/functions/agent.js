const SYSTEM_PROMPT = [
  ...
  "Beginne jede Zusammenfassung mit dem Wichtigsten und nenne danach die entscheidenden Details, Auswirkungen und offenen Punkte.",
  "Antworte vollständig auf Deutsch, außer Eigennamen, Code, Dateinamen oder technische Bezeichner müssen unverändert bleiben.",
...
const providerDefinitions = {
  gemini: { keyEnv: "GEMINI_API_KEY", endpointEnv: "GEMINI_API_URL", modelEnv: "GEMINI_MODEL", defaultModel: "gemini-3.6-flash" },
...
exports.handler = async (event) => {
  ...
  if (!providerDefinitions[provider]) {
    return json(400, { error: "provider muss gemini, gpt, nim oder claude sein." });
  }
  ...
  try {
    if (provider === "claude") {
      ...
      const configuredModel = process.env.GEMINI_MODEL || definition.defaultModel;
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 20000);
      try {
        response = await fetch(`${endpoint}/${configuredModel}:generateContent?key=${encodeURIComponent(apiKey)}`, {
          method: "POST",
    ...
    return json(502, {
      error: `${provider} API ist momentan nicht erreichbar oder das kostenlose Kontingent ist ausgeschöpft.`,
      provider
    });
  }
  ...
  if (!response.ok) {
    ...
    return json(502, {
      error: `${provider} API konnte die Anfrage nicht verarbeiten.`,
      providerStatus: response.status,
      providerType: data?.error?.type || "unknown",
      providerMessage: typeof data?.error?.message === "string" ? data.error.message.slice(0, 300) : "unknown"
    });
  }

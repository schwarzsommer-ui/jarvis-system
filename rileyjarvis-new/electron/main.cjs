...
ipcMain.handle("realtime:has-key", () => Boolean(process.env.OPENAI_API_KEY));

ipcMain.handle("local:chat", async (_event, message) => {
  const response = await fetch("http://127.0.0.1:11434/api/chat", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: "system", content: "You are Ricky, a concise, helpful German desktop assistant. Reply in the user's language." },
  ...
  if (!response.ok) {
    throw new Error(`Lokale KI nicht erreichbar: ${response.status}`);
  }
  const data = await response.json();
  return String(data.message?.content || "Keine Antwort erhalten.");
});
